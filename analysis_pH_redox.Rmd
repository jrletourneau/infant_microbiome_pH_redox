---
title: "Infant fecal pH/redox analysis"
output: html_notebook
---

# Load libraries
```{r}
# Plotting
library(tidyverse)
#library(RColorBrewer)
library(ggtext)
library(ggsignif)
library(cowplot)
library(patchwork)
library(pals)

# Stats
library(phyloseq)
library(lme4)
library(lmerTest)
library(vegan)
library(ALDEx2)

theme_set(theme_bw() +
            theme(axis.text = element_text(color = "black"),
                  axis.ticks = element_line(color = "black"),
                  plot.title = element_text(hjust = 0.5)))

set.seed(123)


```

# Load and clean up data 
```{r}
# pH and redox with clinical data
## See data/variable_descriptions_for_fecal_pH_final.xlsx for explanations of variables
pH_redox <- read.csv("data/fecal_pH_final.csv") %>%
  mutate(postnatalabx = case_when(postnatalabx == "48hr" ~ "48 hr",
                                  postnatalabx == ">5d" ~ ">5 d",
                                  postnatalabx == "None" ~ "None")) %>%
  mutate(postnatalabx = factor(postnatalabx, levels = c("None", "48 hr", ">5 d")))
  

# SCFA data by GC
## Isobutyrate, isovalerate, and valerate were not detected for any samples
## Two samples have been omitted where nothing was detected
## All values have been multiplied by 10 (dilution factor) and are in mM
## Where values were NA/0, they have been replaced with a pseudocount equal to the lowest value observed for that SCFA
gc <- read.csv("data/nicu_gc_cleaned.csv")

# Combine
pH_redox <- pH_redox %>%
  left_join(gc) %>%
  relocate(acetate:total, .after = redox)


```

## Load 16S
```{r}
# Load phyloseq object, omitting samples with < 5000 reads
## This version used to calculate diversity
ps <- readRDS("data/phyloseq.rds") %>%
  prune_samples(sample_sums(.) >= 5000, .)
# 95 taxa by 77 samples (none removed by filter)

# Omit low abundance taxa for differential abundance analysis
ps_filt <- ps %>%
  filter_taxa(function(x) sum(x > 3) > 0.1*length(x), TRUE)
# Reduces to 23 taxa (wow!)

samdf <- as.data.frame(as.matrix(ps@sam_data)) %>%
  mutate(DOL = as.numeric(DOL),
         BGA = as.numeric(BGA),
         pH = as.numeric(pH),
         redox = as.numeric(redox),
         Wt = as.numeric(Wt),
         cga = as.numeric (cga),
         bw = as.numeric(bw))

```

# pH and redox
## Analyze pH and redox in context of clinical data - birth stats
```{r}
# Summarize pH and redox within infants
## Need to account for NA values in some cases
pH_redox_summary <- pH_redox %>%
  group_by(Subject, BGA, bw, matabx, postnatalabx) %>%
  summarize(mean_pH = mean(pH, na.rm = T),
            se_pH = sd(pH, na.rm = T)/sqrt(length(which(!is.na(pH)))),
            mean_redox = mean(redox, na.rm = T),
            se_redox = sd(redox, na.rm = T)/sqrt(length(which(!is.na(redox)))))

# Linear models - pH
pH_redox %>%
  lmer(pH ~ bw + (1|Subject), data = .) %>%
  summary()
# BGA p = 0.014 *
# bw p = 0.0097 **
# matabx p = 0.76

# Examine birth stats
pH_BGA_plot <- ggplot(pH_redox, aes(x = BGA, y = pH, color = Subject)) +
  geom_point() +
  geom_errorbar(data = pH_redox_summary, aes(x = BGA, ymin = mean_pH, ymax = mean_pH, color = Subject),
                width = 0.5, inherit.aes = F) +
  geom_errorbar(data = pH_redox_summary, aes(x = BGA, ymin = mean_pH - se_pH,
                                             ymax = mean_pH + se_pH, color = Subject),
                width = 0.25, inherit.aes = F) +
  labs(x = "Birth gestational age (weeks)", y = "Fecal pH", color = "Participant") +
  geom_textbox(data = data.frame(x=33, y = 7, label = "Linear mixed model<br>pH ~ BGA + (1 | Subject)<br>BGA *p* = 0.014"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), inherit.aes = F)

pH_bw_plot <- ggplot(pH_redox, aes(x = bw, y = pH, color = Subject)) +
  geom_point() +
  geom_errorbar(data = pH_redox_summary, aes(x = bw, ymin = mean_pH, ymax = mean_pH, color = Subject),
                width = 40, inherit.aes = F) +
  geom_errorbar(data = pH_redox_summary, aes(x = bw, ymin = mean_pH - se_pH,
                                             ymax = mean_pH + se_pH, color = Subject),
                width = 20, inherit.aes = F) +
  labs(x = "Birth weight (g)", y = "Fecal pH", color = "Participant") +
  geom_textbox(data = data.frame(x=1220, y = 7, label = "Linear mixed model<br>pH ~ bw + (1 | Subject)<br>bw *p* = 0.0096"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), inherit.aes = F)


# Linear models - redox
pH_redox %>%
  lmer(redox ~ matabx + (1|Subject), data = .) %>%
  summary()
# BGA p = 0.024 *
# bw p = 0.092
# matabx p = 0.87

redox_BGA_plot <- ggplot(pH_redox, aes(x = BGA, y = redox, color = Subject)) +
  geom_point() +
  geom_errorbar(data = pH_redox_summary, aes(x = BGA, ymin = mean_redox,
                                             ymax = mean_redox, color = Subject),
                width = 0.5, inherit.aes = F) +
  geom_errorbar(data = pH_redox_summary, aes(x = BGA, ymin = mean_redox - se_redox,
                                             ymax = mean_redox + se_redox, color = Subject),
                width = 0.25, inherit.aes = F) +
  labs(x = "Birth gestational age (weeks)", y = "Fecal redox potential (mV)", color = "Participant") +
  geom_textbox(data = data.frame(x=32.5, y = -400, label = "Linear mixed model<br>redox ~ BGA + (1 | Subject)<br>BGA *p* = 0.024"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), inherit.aes = F)

redox_bw_plot <- ggplot(pH_redox, aes(x = bw, y = redox, color = Subject)) +
  geom_point() +
  geom_errorbar(data = pH_redox_summary, aes(x = bw, ymin = mean_redox, ymax = mean_redox, color = Subject),
                width = 40, inherit.aes = F) +
  geom_errorbar(data = pH_redox_summary, aes(x = bw, ymin = mean_redox - se_redox,
                                             ymax = mean_redox + se_redox, color = Subject),
                width = 20, inherit.aes = F) +
  labs(x = "Birth weight (g)", y = "Fecal redox potential (mV)", color = "Participant") +
  geom_textbox(data = data.frame(x=1225, y = -420, label = "Linear mixed model<br>redox ~ bw + (1 | Subject)<br>bw *p* = 0.092"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), inherit.aes = F)

# Test BGA-bw correlation
## Doing correlation because I actually only have 11 points here, no longitudinal sampling
cor.test(pH_redox_summary$BGA, pH_redox_summary$bw, method = "spearman")
# p = 0.0079, rho = 0.75

BGA_bw_plot <- ggplot(pH_redox_summary, aes(x = BGA, y = bw, color = Subject)) +
  geom_point() +
  labs(x = "Birth gestational age (weeks)", y = "Birth weight (g)", color = "Participant") +
  geom_textbox(data = data.frame(x=32.5, y = 900, label = "Spearman's &rho; = 0.75<br>*p* = 0.0079"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), inherit.aes = F)

DOL_Wt_plot <- ggplot(pH_redox, aes(x = DOL, y = Wt, color = Subject)) +
  geom_point() +
  geom_line() +
  labs(x = "Age (days)", y = "Weight (g)", color = "Participant")

# pH and redox plot
pH_redox %>%
  lmer(redox ~ pH + (1|Subject), data = .) %>%
  summary()
# p = 0.13

pH_redox_plot <- ggplot(pH_redox, aes(x = pH, y = redox, color = Subject)) +
  geom_point() +
  labs(x = "Fecal pH", y = "Fecal redox potential (mV)", color = "Participant") +
  geom_textbox(data = data.frame(x=6.6, y = -400, label = "Linear mixed model<br>redox ~ pH + (1 | Subject)<br>pH *p* = 0.13"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), inherit.aes = F)

```

## Analyze pH and redox in context of clinical data longitudinally
```{r}
pH_time_plot <- ggplot(pH_redox, aes(x = cga, y = pH, color = Subject)) +
  geom_point() +
  geom_line()

# LMM correcting for bw
pH_redox %>%
  lmer(pH ~ bw + Wt + (1 | Subject), data  = .) %>%
  summary()
# pH
## DOL p = 0.40
## Wt p = 0.63
# redox
## DOL p = 0.0074
## Wt p = 0.16

redox_DOL_plot <- ggplot(pH_redox, aes(x = DOL, y = redox, color = Subject)) +
  geom_point() +
  geom_line(alpha = 0.4) +
  labs(x = "Age (days)", y = "Fecal redox potential (mV)", color = "Participant") +
  geom_textbox(data = data.frame(x = 60, y = -475, label = "Linear mixed model<br>redox ~ bw + DOL + (1 | Subject)<br>DOL *p* = 0.0074"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = unit(3, "inch"), inherit.aes = F)

pH_DOL_plot <- ggplot(pH_redox, aes(x = DOL, y = pH, color = Subject)) +
  geom_point() +
  geom_line(alpha = 0.4) +
  labs(x = "Age (days)", y = "Fecal pH", color = "Participant") +
  geom_textbox(data = data.frame(x = 64, y = 5.42, label = "Linear mixed model<br>pH ~ bw + DOL + (1 | Subject)<br>DOL *p* = 0.40"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = unit(3, "inch"), inherit.aes = F)


```

# SCFA
## Analyze SCFA data in context of clinical data
```{r}
# Gather to make it easier to plot all SCFAs at once
scfa_gather <- pH_redox %>%
  gather(key = scfa, value = conc, acetate:total)

# lmm
scfa_lmm_results <- data.frame(scfa = rep(unique(scfa_gather$scfa), 6),
                               var = rep(c("BGA", "bw", "pH", "redox", "DOL", "Wt"), each = 4),
                               p = NA)

for(r in 1:nrow(scfa_lmm_results)) {
  
  # For longitudinal variables (DOL, Wt), control for birth weight
  if(scfa_lmm_results$var[r] %in% c("DOL", "Wt", "feedintol")) {
    scfa_lmm_results$p[r] <- scfa_gather %>%
      filter(scfa == scfa_lmm_results$scfa[r]) %>%
      lmer(conc ~ bw + get(scfa_lmm_results$var[r]) + (1|Subject), data = .) %>%
      summary() %>%
      .$coefficients %>%
      .[3,5]
      
  } else {
    scfa_lmm_results$p[r] <- scfa_gather %>%
      filter(scfa == scfa_lmm_results$scfa[r]) %>%
      lmer(conc ~ get(scfa_lmm_results$var[r]) + (1|Subject), data = .) %>%
      summary() %>%
      .$coefficients %>%
      .[2,5]
  }

}
# all NS except:
## total ~ pH p = 0.0057, and acetate ~ pH p = 0.0030
## total ~ bw + DOL p = 0.021, AND acetate ~ bw + DOL p = 0.028
## propionate ~ bw + DOL p = 0.020, AND propionate ~ bw + Wt p = 0.00057

# Total/ace and pH
total_pH_plot <- ggplot(pH_redox, aes(x = pH, y = total, color = Subject)) +
  geom_point() +
  labs(x = "Fecal pH", y = "Total SCFA concentration (mM)", color = "Participant") +
  geom_textbox(data = data.frame(x=6.8, y = 120, label = "Linear mixed model<br>total ~ pH + (1 | Subject)<br>pH *p* = 0.0057"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), inherit.aes = F)

ace_pH_plot <- ggplot(pH_redox, aes(x = pH, y = acetate, color = Subject)) +
  geom_point() +
  labs(x = "Fecal pH", y = "Acetate concentration (mM)", color = "Participant") +
  geom_textbox(data = data.frame(x=6.8, y = 120, label = "Linear mixed model<br>acetate ~ pH + (1 | Subject)<br>pH *p* = 0.0030"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), inherit.aes = F)
  
# Total/ace and DOL
total_DOL_plot <- ggplot(pH_redox, aes(x = DOL, y = total, color = Subject)) +
  geom_point() +
  geom_line(alpha = 0.4) +
  labs(x = "Age (days)", y = "Total SCFA concentration (mM)", color = "Participant") +
  geom_textbox(data = data.frame(x = 60, y = 175, label = "Linear mixed model<br>total ~ bw + DOL + (1 | Subject)<br>DOL *p* = 0.021"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = unit(3, "inch"), inherit.aes = F)

ace_DOL_plot <- ggplot(pH_redox, aes(x = DOL, y = acetate, color = Subject)) +
  geom_point() +
  geom_line(alpha = 0.4) +
  labs(x = "Age (days)", y = "Acetate concentration (mM)", color = "Participant") +
  geom_textbox(data = data.frame(x = 60, y = 130, label = "Linear mixed model<br>acetate ~ bw + DOL + (1 | Subject)<br>DOL *p* = 0.028"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = unit(3, "inch"), inherit.aes = F)

# Propionate and DOL / Wt
pro_DOL_plot <- ggplot(pH_redox, aes(x = DOL, y = propionate, color = Subject)) +
  geom_point() +
  geom_line(alpha = 0.4) +
  labs(x = "Age (days)", y = "Propionate concentration (mM)", color = "Participant") +
  geom_textbox(data = data.frame(x = 58, y = 20, label = "Linear mixed model<br>propionate ~ bw + DOL + (1 | Subject)<br>DOL *p* = 0.020"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = unit(3, "inch"), inherit.aes = F)

pro_Wt_plot <- ggplot(pH_redox, aes(x = Wt, y = propionate, color = Subject)) +
  geom_point() +
  geom_line(alpha = 0.4) +
  labs(x = "Weight (g)", y = "Propionate concentration (mM)", color = "Participant") +
  geom_textbox(data = data.frame(x = 1200, y = 20, label = "Linear mixed model<br>propionate ~ bw + Wt + (1 | Subject)<br>Wt *p* = 0.00057"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = unit(3, "inch"), inherit.aes = F)
  

```

## Additional SCFA supp fig
```{r}
#Butyrate against age
but_DOL_plot <- ggplot(pH_redox, aes(x = DOL, y = butyrate, color = Subject)) +
  geom_point() +
  geom_line(alpha = 0.4) +
  labs(x = "Age (days)", y = "Butyrate concentration (mM)", color = "Participant") +
  geom_textbox(data = data.frame(x = 58, y = 20, label = "Linear mixed model<br>butyrate ~ bw + DOL + (1 | Subject)<br>DOL *p* = 0.11"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = unit(3, "inch"), inherit.aes = F)

#But against pH
but_pH_plot <- ggplot(pH_redox, aes(x = pH, y = butyrate, color = Subject)) +
  geom_point() +
  labs(x = "Fecal pH", y = "Butyrate concentration (mM)", color = "Participant") +
  geom_textbox(data = data.frame(x=6.8, y = 20, label = "Linear mixed model<br>butyrate ~ pH + (1 | Subject)<br>pH *p* = 0.084"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), inherit.aes = F)

#Prop against pH
prop_pH_plot <- ggplot(pH_redox, aes(x = pH, y = propionate, color = Subject)) +
  geom_point() +
  labs(x = "Fecal pH", y = "Propionate concentration (mM)", color = "Participant") +
  geom_textbox(data = data.frame(x=6.8, y = 20, label = "Linear mixed model<br>propionate ~ pH + (1 | Subject)<br>pH *p* = 0.31"),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = 3, inherit.aes = F)


scfa_supp <- (but_DOL_plot + theme(legend.position = "none")) /
  but_pH_plot / (prop_pH_plot + theme(legend.position = "none")) +
  plot_annotation(tag_levels = 'A')

ggsave("plots/fig_s3.png", scfa_supp, height = 10, width = 6)

```

# 16S
## Analyze 16S data in context of clinical data
```{r}

# Prep scfa and matabx vars to add into tests

scfa_spread <- scfa_gather %>%
  mutate(conc = case_when(is.na(conc) ~ 0, T ~ conc)) %>%
  filter(scfa != "total") %>%
  dplyr::select(Sample, postnatalabx, scfa, conc) %>%
  spread(key = scfa, value = conc)


# Diversity
diversity <- ps %>%
  estimate_richness(split = TRUE, measures = c("Shannon", "Observed")) %>%
  cbind(ps@sam_data) %>%
  left_join(scfa_spread) %>%
  gather(key = measure, value = value, c("Shannon", "Observed"))

diversity_summary <- diversity %>%
  group_by(Subject_ID, bw, BGA, measure) %>%
  summarize(mean = mean(value, na.rm = T),
            se = sd(value, na.rm = T)/sqrt(length(which(!is.na(value)))))

## lmer
diversity %>%
  filter(measure == "Shannon") %>%
  lmer(value ~ BGA + (1 | Subject_ID), data = .) %>%
  summary()
# Shannon bw = 0.61, BGA = 0.23
# Observed bw = 0.75, BGA = 0.50
# Shannon bw + DOL p = 9.06e-06
# Observed bw + DOL p = 0.025

diversity %>%
  filter(measure == "Observed") %>%
  lmer(value ~ bw + DOL + pH + redox + postnatalabx + acetate + butyrate + propionate + (1 | Subject_ID), data = .) %>%
  summary()
# Shannon
## postnatalabx48hr -0.8053307  0.2943897  7.6788534  -2.736 0.026628 *  
## postnatalabx>5d  -1.2367348  0.3870977  7.3246350  -3.195 0.014279 *
# Observed - NS

## Diversity plot
diversity_bw_plot <- ggplot(diversity, aes(x = bw, y = value, color = Subject_ID)) +
  geom_point() +
  geom_errorbar(data = diversity_summary, aes(x = bw, ymin = mean, ymax = mean, color = Subject_ID),
                width = 40, inherit.aes = F) +
  geom_errorbar(data = diversity_summary, aes(x = bw, ymin = mean - se,
                                             ymax = mean + se, color = Subject_ID),
                width = 20, inherit.aes = F) +
  facet_wrap(~measure, scales = "free_y") +
  labs(x = "Birth weight (g)", y = "Alpha diversity", color = "Participant") +
  geom_textbox(data = data.frame(x = c(1100, 1220), y = c(23, 0.5),
                                 measure = c("Observed", "Shannon"),
                                 label = c("Linear mixed model<br>observed ~ bw + (1 | Subject)<br>bw *p* = 0.75", "Linear mixed model<br>Shannon ~ bw + (1 | Subject)<br>bw *p* = 0.61")),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = unit(3, "inch"), inherit.aes = F)
  

diversity_DOL_plot <- ggplot(diversity, aes(x = DOL, y = value,
                                            color = Subject_ID, group = Subject_ID)) +
  geom_point() +
  geom_line(alpha = 0.4) +
  facet_wrap(~measure, scales = "free_y") +
  labs(x = "Age (days)", y = "Alpha diversity", color = "Participant") +
  geom_textbox(data = data.frame(x = c(50, 55), y = c(23, 0.11),
                                 measure = c("Observed", "Shannon"),
                                 label = c("Linear mixed model<br>observed ~ bw + DOL + (1 | Subject)<br>DOL *p* = 0.025", "Linear mixed model<br>Shannon ~ bw + DOL + (1 | Subject)<br>DOL *p* = 9.1 x 10<sup>-6</sup>")),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = unit(3, "inch"), inherit.aes = F)

diversity_abx_summary <- diversity %>%
  dplyr::select(postnatalabx, measure, value) %>%
  group_by(postnatalabx, measure) %>%
  summarize(mean = mean(value), se = sd(value)/sqrt(length(value)))

div_abx_plot <- ggplot(diversity, aes(x = postnatalabx, y = value, color = Subject_ID)) +
  geom_point(position = position_jitter(width = 0.05, height = 0)) +
  geom_errorbar(data = diversity_abx_summary,
                aes(x = postnatalabx, ymin = mean, ymax = mean),
                width = 0.5, color = "red", inherit.aes = F) +
  geom_errorbar(data = diversity_abx_summary, aes(x = postnatalabx, ymin = mean - se,
                                             ymax = mean + se),
                width = 0.25, inherit.aes = F) +
  geom_text(data = data.frame(measure = "Shannon", x = c("48 hr", ">5 d"),
                              y = c(2, 1.3), label = c("*", "*")),
            mapping = aes(x = x, y = y, label = label), size = 10, inherit.aes = F) +
  labs(x = "Duration of postnatal antibiotics",
       y = "Alpha diversity", color = "Participant") +
  facet_wrap(~measure, scales = "free_y")


# NMDS
bray <- distance(ps, method = "bray") %>% as.matrix()

## Test effect of participant
adonis2(formula = bray ~ Subject_ID, data = samdf,
        permutations=9999, method = "bray")
# participant p < 0.0001

# More complicated model
adonis2(formula = bray ~ bw + DOL + pH + redox + Subject_ID, data = samdf,
        permutations=9999, method = "bray", na.action = na.omit)
#           Df SumOfSqs      R2       F Pr(>F)    
#bw          1   2.6453 0.10374 17.6512 0.0001 ***
#DOL         1   2.0974 0.08225 13.9951 0.0001 ***
#pH          1   0.4445 0.01743  2.9659 0.0073 ** 
#redox       1   0.4637 0.01818  3.0942 0.0054 ** 
#Subject_ID  9  10.5577 0.41403  7.8276 0.0001 ***

ord <- ordinate(ps, method="NMDS", distance="bray")

ord_plot <- plot_ordination(ps, ord, color="Subject_ID", title=NULL) +
  stat_ellipse(linetype = "dashed") +
  labs(color = "Participant") +
   geom_textbox(data = data.frame(x = 3, y = 1.25,
                                  label = paste("PERMANOVA",
                                                "bw R<sup>2</sup> = 0.10, *p* < 0.0001",
                                                "DOL R<sup>2</sup> = 0.082, *p* < 0.0001",
                                                "pH R<sup>2</sup> = 0.017, *p* = 0.0073",
                                                "redox R<sup>2</sup> = 0.018, *p* = 0.0054",
                                                "participant R<sup>2</sup> = 0.41, *p* < 0.0001",
                                                sep = "<br>")),
               aes(x, y, label = label), halign = 0.5, fill = NA, box.colour = NA,
               box.padding = grid::unit(rep(0, 4), "pt"), width = unit(3, "inch"), inherit.aes = F)


```

## ALDEx2 - Look at specific taxa with ALDEx2
```{r}
# ALDEx2 GLM incorporating multiple variables
## Using ps_filt here!
otu_flip <- as.data.frame(t(ps_filt@otu_table))
mm <- samdf[rownames(as.data.frame(as.matrix(ps_filt@sam_data))),] %>%
  model.matrix(~ bw + DOL + pH + redox + Subject_ID, data = .)

# ALDEx2 correlation (not incorporating participant...)
clr <- aldex.clr(otu_flip[,colnames(otu_flip) %in% rownames(mm)], mm, mc.samples = 128, denom ="all")
glm.test <- aldex.glm(clr) # slow
#write.csv(glm.test, "plots/aldex2_glm_bw_DOL_pH_redox_participant.csv")
# bw seq_1 p.BH = 0.006447178, seq_4 p.BH = 0.024139623;
# DOL seq_1 p.BH = 0.0003955539;
# pH and redox none

melty <- ps_filt %>%
  microbiome::transform(transform = "compositional") %>%
  psmelt() %>%
  mutate(OTU = sub("seq_", "ASV ", OTU)) %>%
  mutate(binom = case_when(is.na(Genus) ~ paste0(Family, " family (", OTU, ")"),
                           !is.na(Genus) & is.na(Species) ~
                             paste0("*", Genus, " sp.* (", OTU, ")"),
                           !is.na(Species) ~
                             paste0("*", Genus, " ", Species, "* (", OTU, ")"))) %>%
  filter(OTU %in% c("ASV 1", "ASV 4")) %>%
  mutate(binom = factor(binom,
                        levels = c("*Staphylococcus sp.* (ASV 1)",
                                   "*Escherichia-Shigella sp.* (ASV 4)")))

melty_summary <- melty %>%
  group_by(OTU, binom, Subject_ID, bw) %>%
  summarize(mean = mean(Abundance), se = sd(Abundance)/sqrt(length(Abundance)))

tax_bw_plot <- melty %>%
  filter(OTU %in% c("ASV 1", "ASV 4")) %>%
  ggplot(aes(x = bw, y = Abundance, color = Subject_ID)) +
  geom_point() +
  geom_errorbar(data = filter(melty_summary, OTU %in% c("ASV 1", "ASV 4",
                                                        "ASV 10", "ASV 11")),
                aes(x = bw, ymin = mean, ymax = mean, color = Subject_ID),
                width = 40, inherit.aes = F) +
  geom_errorbar(data = filter(melty_summary, OTU %in% c("ASV 1", "ASV 4",
                                                        "ASV 10", "ASV 11")),
                aes(x = bw, ymin = mean - se, ymax = mean + se, color = Subject_ID),
                width = 20, inherit.aes = F) +
  labs(x = "Birth weight (g)", y = "Relative abundance", color = "Participant") +
  facet_wrap(~binom, scales = "free_y", nrow = 1) +
  theme(strip.text = element_markdown())

tax_DOL_plot <- melty %>%
  filter(OTU %in% c("ASV 1")) %>%
  ggplot(aes(x = DOL, y = Abundance, color = Subject_ID)) +
  geom_point() +
  geom_line() +
  labs(x = "Age (days)", y = "Relative abundance", color = "Participant") +
  facet_wrap(~binom, scales = "free_y") +
  theme(strip.text = element_markdown())



# Also do for genus/family
ps_genus_filt <- ps %>%
  tax_glom(taxrank = "Genus") %>%
  filter_taxa(function(x) sum(x > 3) > 0.1*length(x), TRUE)
# 16 genera

otu_flip_gen <- as.data.frame(t(ps_genus_filt@otu_table))
mm_gen <- samdf[rownames(as.data.frame(as.matrix(ps_genus_filt@sam_data))),] %>%
  model.matrix(~ bw + DOL + pH + redox + Subject_ID, data = .)

# ALDEx2 correlation (not incorporating participant...)
clr_gen <- aldex.clr(otu_flip_gen[,colnames(otu_flip_gen) %in% rownames(mm_gen)],
                     mm_gen, mc.samples = 128, denom ="all")
glm.test_gen <- aldex.glm(clr_gen) # slow


## Family
ps_fam_filt <- ps %>%
  tax_glom(taxrank = "Family") %>%
  filter_taxa(function(x) sum(x > 3) > 0.1*length(x), TRUE)
# 12 families

otu_flip_fam <- as.data.frame(t(ps_fam_filt@otu_table))
mm_fam <- samdf[rownames(as.data.frame(as.matrix(ps_fam_filt@sam_data))),] %>%
  model.matrix(~ bw + DOL + pH + redox + Subject_ID, data = .)

# ALDEx2 correlation (not incorporating participant...)
clr_fam <- aldex.clr(otu_flip_fam[,colnames(otu_flip_fam) %in% rownames(mm_fam)],
                     mm_fam, mc.samples = 128, denom ="all")
glm.test_fam <- aldex.glm(clr_fam) # slow


melty_fam <- ps_fam_filt %>%
  microbiome::transform(transform = "clr") %>%
  psmelt()

fam_bw_plot <- melty_fam %>%
  filter(OTU %in% c("seq_1", "seq_8", "seq_17")) %>%
  ggplot(aes(x = bw, y = Abundance, color = Subject_ID)) +
  geom_point() +
  labs(x = "Birth weight (g)", y = "Relative abundance") +
  facet_wrap(~Family, scales = "free_y")

fam_DOL_plot <- melty_fam %>%
  filter(OTU %in% c("seq_1", "seq_3", "seq_8")) %>%
  ggplot(aes(x = DOL, y = Abundance, color = Subject_ID)) +
  geom_point() +
  geom_line() +
  labs(x = "Age (days)", y = "Relative abundance") +
  facet_wrap(~Family, scales = "free_y")

fam_redox_plot <- melty_fam %>%
  filter(OTU %in% c("seq_2")) %>%
  ggplot(aes(x = redox, y = Abundance, color = Subject_ID)) +
  geom_point() +
  geom_line() +
  labs(x = "Redox potential (mV)", y = "Relative abundance") +
  facet_wrap(~Family, scales = "free_y")

```

## ALDEx2 - more complex model
bw + DOL + pH + redox + postnatalabx + acetate + butyrate + propionate + Subject_ID
```{r}
# ALDEx2 GLM incorporating multiple variables
## Using ps_filt here!
otu_flip <- as.data.frame(t(ps_filt@otu_table))
mm <- samdf[rownames(as.data.frame(as.matrix(ps_filt@sam_data))),] %>%
  rownames_to_column("temp_col") %>%
  left_join(scfa_spread) %>%
  column_to_rownames("temp_col") %>%
  model.matrix(~ bw + DOL + pH + redox + postnatalabx + acetate + butyrate + propionate + Subject_ID, data = .)

# ALDEx2 correlation (not incorporating participant...)
clr <- aldex.clr(otu_flip[,colnames(otu_flip) %in% rownames(mm)], mm, mc.samples = 128, denom ="all")
glm.test2 <- aldex.glm(clr) # slow
 #write.csv(glm.test2, "plots/aldex2_complicated_model.csv")
# DOL: seq_1 p.BH = 0.001435765; propionate seq_12 p.BH = 0.01612447

melty_vei <- ps_filt %>%
  microbiome::transform(transform = "compositional") %>%
  psmelt() %>%
  mutate(OTU = sub("seq_", "ASV ", OTU)) %>%
  mutate(binom = case_when(is.na(Genus) ~ paste0(Family, " family (", OTU, ")"),
                           !is.na(Genus) & is.na(Species) ~
                             paste0("*", Genus, " sp.* (", OTU, ")"),
                           !is.na(Species) ~
                             paste0("*", Genus, " ", Species, "* (", OTU, ")"))) %>%
  filter(OTU %in% c("ASV 12")) %>%
  mutate(binom = factor(binom,
                        levels = c("*Veillonella sp.* (ASV 12)"))) %>%
  left_join(scfa_spread, by = c("sample_Sample" = "Sample"))

pro_vei_plot <- ggplot(melty_vei, aes(x = propionate, y = Abundance, color = Subject_ID)) +
  geom_point() +
  labs(x = "Propionate concentration (mM)", y = "Relative abundance", color = "Participant") +
  facet_wrap(~binom, scales = "free_y") +
  theme(strip.text = element_markdown())


```


## Bar plot
```{r}
# Let's do it at the genus level
# Retain only genera that pass the sum(x > 3) > 0.1*length(x) filter, but include others in relative abundance calculation
melty_all <- ps %>%
  tax_glom(taxrank = "Genus") %>%
  #filter_taxa(function(x) sum(x > 3) > 0.1*length(x), TRUE) %>%
  microbiome::transform(transform = "compositional") %>%
  psmelt() %>%
  mutate(OTU = sub("seq_", "ASV ", OTU)) %>%
  mutate(binom = paste0(Genus, " (", OTU, ")")) %>%
  filter(OTU %in% c("ASV 1", "ASV 4", "ASV 2", "ASV 3", "ASV 5", "ASV 6",
                    "ASV 8", "ASV 7", "ASV 9", "ASV 12", "ASV 20", "ASV 17",
                    "ASV 13", "ASV 15", "ASV 30", "ASV 26"))

genera <- unique(melty_all$Genus)

average_abundance <- melty_all %>%
  group_by(Subject_ID, Genus) %>%
  summarize(Avg_Rel_Abundance = mean(Abundance)) %>%
  ungroup() %>%
  as.data.frame() %>%
  spread(key = Genus, value = Avg_Rel_Abundance) %>%
  mutate(Other = 1 - rowSums(.[,2:ncol(.)])) %>%
  gather(key = Genus, value = Avg_Rel_Abundance, 2:ncol(.)) %>%
  filter(Avg_Rel_Abundance > 0) %>%
  mutate(Genus = factor(Genus, levels = c(genera, "Other")))

bar_plot <- ggplot(average_abundance, aes(x = Subject_ID,
                                          y = Avg_Rel_Abundance,
                                          fill = Genus)) +
  geom_bar(stat = "identity", color = "black") +
  labs(x = "Subject", y = "Average relative abundance", fill = "Genus") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.text = element_markdown()) +
  scale_fill_manual(values = setNames(rev(pals::polychrome(n = 17)),
                                      levels(average_abundance$Genus)))

#ggsave("plots/genus_bar_plot.png", bar_plot, height = 5, width = 7)


```


# Antibiotics
```{r}
# Some infants had abx during the study, which is not reflected in the postnatalabx variable
## Just making a note of those here
pH_redox_init_abx <- pH_redox %>%
  filter(postnatalabx != "None") %>%
  filter(!(Subject == "PH02" & DOL > 24) &
           !(Subject == "PH05" & DOL > 30) &
           !(Subject == "PH09" & DOL > 29))

pH_redox %>%
  lmer(pH ~ bw + postnatalabx + (1 | Subject), data  = .) %>%
  summary()

# pH
#                   Estimate Std. Error         df t value Pr(>|t|)    
#(Intercept)       6.5745976  0.3585911 73.0000000  18.335  < 2e-16 ***
#bw               -0.0007185  0.0002952 73.0000000  -2.434   0.0174 *  
#postnatalabx48hr  0.7280109  0.1549805 73.0000000   4.697 1.21e-05 ***
#postnatalabx>5d   1.0677934  0.1932924 73.0000000   5.524 4.84e-07 ***

# redox
#                  Estimate Std. Error        df t value Pr(>|t|)  
#(Intercept)      -461.2496   146.9521    9.7776  -3.139   0.0108 *
#bw                  0.3103     0.1184   10.7546   2.621   0.0242 *
#postnatalabx48hr  131.0291    68.1192    7.5177   1.924   0.0930 .
#postnatalabx>5d   143.7248    84.3523    7.0578   1.704   0.1318 

abx_summary <- pH_redox_summary %>%
  group_by(postnatalabx) %>%
  summarize(mean_pH2 = mean(mean_pH), se_pH2 = sd(mean_pH)/sqrt(length(mean_pH)),
            mean_redox2 = mean(mean_redox), se_redox2 = sd(mean_redox)/sqrt(length(mean_redox))) %>%
  mutate(postnatalabx = factor(postnatalabx, levels = c("None", "48 hr", ">5 d")))

abx_pH_plot <- ggplot(pH_redox, aes(x = postnatalabx, y = pH, color = Subject)) +
  geom_point(position = position_jitter(width = 0.05, height = 0)) +
  geom_errorbar(data = abx_summary, aes(x = postnatalabx, ymin = mean_pH2, ymax = mean_pH2),
                width = 0.5, color = "red", inherit.aes = F) +
  geom_errorbar(data = abx_summary, aes(x = postnatalabx, ymin = mean_pH2 - se_pH2,
                                             ymax = mean_pH2 + se_pH2),
                width = 0.25, inherit.aes = F) +
  labs(x = "Duration of postnatal antibiotics", y = "Fecal pH", color = "Participant") +
  annotate(geom = "text", x = c(2,3), y = c(7.85, 7.85), label = c("***", "***"), size = 10)

abx_redox_plot <- ggplot(pH_redox, aes(x = postnatalabx, y = redox, color = Subject)) +
  geom_point(position = position_jitter(width = 0.05, height = 0)) +
  geom_errorbar(data = abx_summary, aes(x = postnatalabx, ymin = mean_redox2, ymax = mean_redox2),
                width = 0.5, color = "red", inherit.aes = F) +
  geom_errorbar(data = abx_summary, aes(x = postnatalabx, ymin = mean_redox2 - se_redox2,
                                             ymax = mean_redox2 + se_redox2),
                width = 0.25, inherit.aes = F) +
  labs(x = "Duration of postnatal antibiotics", y = "Fecal redox potential (mV)",
       color = "Participant")

```

## Days off abx
```{r}

# new linear mixed model that includes both the duration of initial antibiotic exposure and days since last antibiotic treatment (accounting for a second treatment, if applicable), while controlling for birth weight and day of life

pH_redox %>%
  lmer(pH ~ bw + DOL + postnatalabx + daysoffabx + (1 | Subject), data  = .) %>%
  summary()
# pH
#                    Estimate Std. Error         df t value Pr(>|t|)    
#(Intercept)        6.5541215  0.4815699 60.0000000  13.610  < 2e-16 ***
#bw                -0.0006867  0.0003388 60.0000000  -2.027 0.047131 *  
#DOL                0.0007676  0.0039029 60.0000000   0.197 0.844750    
#postnatalabx48 hr  0.6526068  0.2401244 60.0000000   2.718 0.008578 ** 
#postnatalabx>5 d   0.9499408  0.2710534 60.0000000   3.505 0.000871 ***
#daysoffabx         0.0033978  0.0055932 60.0000000   0.607 0.545814    

pH_redox %>%
  lmer(redox ~ bw + DOL + postnatalabx + daysoffabx + (1 | Subject), data  = .) %>%
  summary()
# redox
#                   Estimate Std. Error        df t value Pr(>|t|)  
#(Intercept)       -313.6694   170.4525    8.0486  -1.840   0.1028  
#bw                   0.2172     0.1197    6.4813   1.815   0.1158  
#DOL                 -2.1379     1.2162   35.4673  -1.758   0.0874 .
#postnatalabx48 hr  154.7750    89.0341    6.8562   1.738   0.1266  
#postnatalabx>5 d   220.6198    98.8093    6.1140   2.233   0.0662 .
#daysoffabx          -2.2078     1.7638   29.1823  -1.252   0.2206  

library(scales)


## Here we will use all points
DOL_pH_byAbx_plot <- pH_redox %>%
  mutate(Subject = factor(Subject,
                          levels = c("PH02", "PH11",
                                     "PH03", "PH05", "PH06", "PH07",
                                     "PH08", "PH09", "PH10",
                                     "PH01", "PH04"))) %>%
  mutate(onabx = case_when(is.na(daysoffabx) ~ "Off antibiotics",
                           daysoffabx == 0 ~ "On antibiotics",
                           daysoffabx != 0 ~ "Off antibiotics")) %>%
  ggplot(aes(x = DOL, y = pH, color = onabx, group = Subject)) +
  geom_line(color = "black") +
  geom_point() +
  scale_color_manual(values = c("black", "red")) +
  labs(x = "Age (days)", y = "pH", color = "On antibiotics?") +
  facet_wrap(~Subject*postnatalabx, scales = "free_x") +
  scale_x_continuous(breaks= pretty_breaks())
  

DOL_redox_byAbx_plot <- pH_redox %>%
  mutate(Subject = factor(Subject,
                          levels = c("PH02", "PH11",
                                     "PH03", "PH05", "PH06", "PH07",
                                     "PH08", "PH09", "PH10",
                                     "PH01", "PH04"))) %>%
  filter(!is.na(redox)) %>%
  mutate(onabx = case_when(is.na(daysoffabx) ~ "Off antibiotics",
                           daysoffabx == 0 ~ "On antibiotics",
                           daysoffabx != 0 ~ "Off antibiotics")) %>%
  ggplot(aes(x = DOL, y = redox, color = onabx, group = Subject)) +
  geom_line(color = "black") +
  geom_point() +
  scale_color_manual(values = c("black", "red")) +
  labs(x = "Age (days)", y = "Redox potential (mV)", color = "On antibiotics?") +
  facet_wrap(~Subject*postnatalabx, scales = "free_x") +
  scale_x_continuous(breaks= pretty_breaks())

DOL_Abx_plots <- (DOL_pH_byAbx_plot + theme(legend.position = "bottom")) /
  (DOL_redox_byAbx_plot + theme(legend.position = "none")) +
  plot_annotation(tag_levels = 'A')

#ggsave("plots/fig_s_abx.png", DOL_Abx_plots, height = 10, width = 7)



daysoffabx_pH_plot <- ggplot(pH_redox, aes(x = daysoffabx, y = pH, color = Subject)) +
  geom_point() +
  labs(x = "Time since last antibiotics (days)", y = "pH", color = "Subject") +
  facet_wrap(~postnatalabx)

daysoffabx_redox_plot <- ggplot(pH_redox, aes(x = daysoffabx, y = redox, color = Subject)) +
  geom_point() +
  labs(x = "Time since last antibiotics (days)", y = "Redox potential (mV)",
       color = "Subject") +
  facet_wrap(~postnatalabx)

daysoffabx_plots <- daysoffabx_pH_plot /
  daysoffabx_redox_plot +
  plot_annotation(tag_levels = 'A')

ggsave("plots/fig_s_abx2.png", daysoffabx_plots, height = 8, width = 8)

```



# Patch figures
```{r}
# Fig. 1 - birth stats and pH/redox
fig1 <- plot_grid(pH_bw_plot + theme(legend.position = "none"),
                  redox_bw_plot + theme(legend.position = "none"),
                  pH_BGA_plot + theme(legend.position = "none"),
                  redox_BGA_plot + theme(legend.position = "none"),
                  pH_DOL_plot + theme(legend.position = "none"),
                  redox_DOL_plot + theme(legend.position = "none"),
                  ncol = 2,
                  labels = c("A", "B", "C", "D", "E", "F"), label_fontface = "plain") %>%
  plot_grid(., get_legend(pH_BGA_plot), rel_widths = c(8,1))
#ggsave("plots/fig1.png", fig1, height = 10, width = 10)

# Fig. 2 - SCFA
fig2 <- plot_grid(total_pH_plot + theme(legend.position = "none"),
                  ace_pH_plot + theme(legend.position = "none"),
                  total_DOL_plot + theme(legend.position = "none"),
                  ace_DOL_plot + theme(legend.position = "none"),
                  pro_DOL_plot + theme(legend.position = "none"),
                  pro_Wt_plot + theme(legend.position = "none"),
                  ncol = 2,
                  labels = c("A", "B", "C", "D", "E", "F"), label_fontface = "plain") %>%
  plot_grid(., get_legend(total_pH_plot), rel_widths = c(8,1))
#ggsave("plots/fig2.png", fig2, height = 10, width = 10)

# Fig. 3 16S
fig_16s_comm <- plot_grid(diversity_DOL_plot + theme(legend.position = "none"),
                          div_abx_plot + theme(legend.position = "none"),
                          ord_plot,
                          ncol = 1, labels = c("A", "B", "C"),
                          label_fontface = "plain")
ggsave("plots/fig_16S_comm.png", fig_16s_comm, height = 10.5, width = 8)

fig_16s_taxa <- plot_grid(tax_DOL_plot + theme(legend.position = "none"),
                          pro_vei_plot + theme(legend.position = "none"),
                          ncol = 2, labels = c("C", "D"),
                          label_fontface = "plain") %>%
  plot_grid(tax_bw_plot + theme(legend.position = "none"), .,
            ncol = 1, labels = c("B", ""),
            label_fontface = "plain") %>%
  plot_grid(., get_legend(diversity_bw_plot),
            nrow = 1, rel_widths = c(10,1)) %>%
  plot_grid(bar_plot, .,
            ncol = 1, rel_heights = c(100,130),
            labels = c("A", ""), label_fontface = "plain")
  
ggsave("plots/fig_16S_taxa.png", fig_16s_taxa, height = 12, width = 8)




# Fig. S1 
figS1 <- plot_grid(BGA_bw_plot + theme(legend.position = "none"),
                   pH_redox_plot  + theme(legend.position = "none"),
                   get_legend(pH_redox_plot),
                   nrow = 1, rel_widths = c(5,5,1),
                   labels = c("A", "B", ""), label_fontface = "plain")
#ggsave("plots/figS1.png", figS1, height = 4, width = 8)

# Fig. S2
figS2 <- plot_grid(diversity_bw_plot,
                   nrow = 1, labels = c(""), label_fontface = "plain")
#ggsave("plots/figS2.png", figS2, height = 4, width = 10)


# New Abx figure
fig_abx <- plot_grid(abx_pH_plot + theme(legend.position = "none"),
                     abx_redox_plot + theme(legend.position = "none"),
                     get_legend(abx_redox_plot),
                     nrow = 1, rel_widths = c(5, 5, 1),
                     labels = c("A", "B", ""), label_fontface = "plain")

#ggsave("plots/fig_abx.png", fig_abx, height = 4, width = 8)


```